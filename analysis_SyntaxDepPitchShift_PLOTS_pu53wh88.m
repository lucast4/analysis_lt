...
% PLOT DEVIATION FROM BASELINE FOR EACH syl
figure; hold on;
H=[];
for j=1:num_syl;
    X=[];
    ...
    for i=1:num_days; % number of days
        try
            X(i)=all_days_various.lt_calc_day_pitch{i}.FF.(syllables{j}).mean_FF;
            XminusBaseline(i)=X(i)-baseline_mean;
            Xzscore(i)=XminusBaseline(i)/baseline_std;
        catch err
            X(i)=nan;
            XminusBaseline(i)=nan;
            Xzscore(i)=nan;
            continue
        end
    end
    H(j)=plot(Xzscore,'o-','Color',plot_colors{j},'MarkerFaceColor',plot_colors{j},'MarkerSize',7);
    line(xlim,[0 0]);
...
title('Deviation from mean baseline pitch (z-score)');
xlabel('days');
ylabel('Deviation from baseline (z-score)');
...
for jj=NonTargSyl;
    ...
    ylabel(['Non-target: ' motif_table(jj,1)]);
    % linear regression
    [B,Bint,R,Rint,Stats]=regress(Xdiff_lag(:,jj),[ones(size(Xdiff,1)-1,1) Xdiff(1:NewNumDays_lag,TargSyl)]);
    R2=Stats(1);
    xlimits=xlim;
    ylimits=ylim;
    Y(1)=B(2)*xlimits(1)+B(1);
...
for jj=NonTargSyl;
    ...
    ylabel(['Non-target: ' motif_table(jj,1)]);
    % linear regression
    [B,Bint,R,Rint,Stats]=regress(Xdiff(LearningDays-1,jj),[ones(size(LearningDays,2),1) Xdiff(LearningDays-1,TargSyl)]);
    R2=Stats(1);
    xlimits=xlim;
    ylimits=ylim;
    Y(1)=B(2)*xlimits(1)+B(1);
...
for jj=SylsToCombine;
    for kk=1:num_days;
        try % sometimes lacking day data
...
for i=1:num_days;
    ...
    Ycombined.DayMeans(i)=nan;
    Ycombined.DaySD(i)=nan;
    Ycombined.DaySE(i)=nan;
...
for i=1:num_days;
    ...
    Ycombined2.DaySE(i)=Ycombined2.DaySD(i)/sqrt(size(Ycombined2.raw{i},1));
     ...
     Ycombined2.DayMeans(i)=nan;
    Ycombined2.DaySD(i)=nan;
    Ycombined2.DaySE(i)=nan;
    end       
...
h=errorbar(Ycombined.DayMeans,Ycombined.DaySE,'o-','Color',plot_colors{2},'MarkerFaceColor',plot_colors{2},'MarkerSize',7);
errorbar_tick(h,200);

legend(motif_table{4,1}, motif_table{5,1},'Other b: (abb. abbb. cbb)');
title('Mean Pitch (+/- SEM) over days');
xlabel('days');
ylabel('Pitch (hz)');
...
legend(motif_table{1,1}, motif_table{2,1},motif_table{4,1});
title('Mean Pitch (+/- SEM) over days');
xlabel('days');
ylabel('Pitch (hz)');
...
legend(motif_table{3,1}, motif_table{5,1});
title('Mean Pitch (+/- SEM) over days');
xlabel('days');
ylabel('Pitch (hz)');
...
% Plot all syllables combined
figure; hold on;
h=errorbar(Ycombined2.DayMeans,Ycombined2.DaySE,'o-','Color','k','MarkerFaceColor','k','MarkerSize',7);
errorbar_tick(h,200);

legend('All b combined');

title('Pitch (mean +/- SEM) over days');
xlabel('days');
ylabel('Pitch (hz)');
ylim([3250 3650])
