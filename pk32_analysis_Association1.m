%% CHECKING TEMPLATES (care about note groups)

% ============================== TO ASK WHETHER THERE IS EFFECT OF LASER
% Analyze as normally would by combining all songs into one batch, and comparing stim on to stim off trials. No need to actually use
% note group log.

% 1) Label all songs using autolabel
% 2) Run this
% lt_sort_batch_by_labeled_MULT('batch.NoteGroup_0.rec_FB', 'batch.NoteGroup_1.rec_FB');


% ------ (WNup, note 1) [notegroup 0]
clear all; close all;
batchf= 'batch.NoteGroup_0.rec_FB.LABELED';
get_WN_hits=1;
...
syl_pre = 'dc';
syl_post = '';
% config='/bluejay4/lucas/birds/pk32/100415_Association1_preWN_STIMon/config.evconfig2';
% config='/bluejay4/lucas/birds/pk32/100615_Association1_durWN_STIMon_day1/config.evconfig2';
config='/bluejay4/lucas/birds/pk32/100815_Association1_durWN_STIMon_day3/config_100815.evconfig2';
NoteNum = 1; 

lt_check_hit_templ_freq_v2_EvTAFv4Sim(batchf, syl, syl_pre, syl_post, get_WN_hits, get_offline_match, get_FF, config, NoteNum)


% ------- (WNdn, note 2) [notegroup 1]
clear all; close all;
batchf= 'batch.NoteGroup_1.rec_FB.LABELED';
get_WN_hits=1;
get_offline_match=1; % do offline matching using template? (ADDX=1)
get_FF=1; % Analyze FF using offline matching?
syl = 'c';
...
%% CHECKING TEMPLATES (all songs, ignore note group)
...
syl_pre = 'd';
syl_post = '';
config= '/bluejay4/lucas/birds/pk32/config_100115_2.evconfig2'; 
NoteNum = 0; 
...
[fnames, sylnum]=lt_jc_chcklbl(batch,syl.targ, 0.025,0.025,syl.pre,'','');
[vlsorfn vlsorind]=jc_vlsorfn(batch,syl.targ,syl.pre,'');
