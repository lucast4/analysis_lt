%% ADULT - summary figures
clear all
ADAAind='25May2014_1630';
...
con_div = 'divergence';
all_days_all_analysis=lt_all_days_PLOT_AllTrans([1 2], '28Apr2014_1940', con_div);

con_div = 'convergence';
all_days_all_analysis=lt_all_days_PLOT_AllTrans([1 2], '28Apr2014_1940', con_div);


%% 4/28 - SUMMARY PLOTS for CP seq

% PLOT transition matrices and difference matrices for all days
con_div = 'divergence';
all_days_all_analysis=lt_all_days_PLOT_AllTrans([1 2 3], '26Mar2014_1804', con_div);
...
clear all; close all;
load /bluejay3/lucas/birds/bk73bk61/all_days_all_analysis/all_days_all_analysis_26Mar2014_1804_CPseq_AllTrans_WNtrigLabel
...
for kk=1:length(syl1_list);
    ...
    % make syl colors
    syl_color=lt_make_plot_colors(length(syl2_list),0);
...
for kk=1:length(syl2_list)
    ...
    % make syl colors
    syl_color=lt_make_plot_colors(length(syl1_list),0);
...
%% 3/26/14 - analysis of 1) transitions, 2) hit rate, and 3) collecting labels

lt_all_days_various_calculations % say yes for transitions, hit rate, and labels
